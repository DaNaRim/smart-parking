...
public class ParkingServiceImpl implements ParkingService {
  ...
  public ParkingServiceImpl(
      ParkingDao parkingDao, ParkingPlaceDao parkingPlaceDao, UserService userService) {
    ...
    for (Parking parking : parkingList) {
      ...
      result.add(
          new FreeParkResponse(
              ...
              Math.round(
                      Math.sqrt(Math.pow(parking.getX() - x, 2) + Math.pow(parking.getY() - y, 2))
                          * 100.0)
                  / 100.0 // count distance
              ));
    }
    return result;
  }
  ...
  public void bookPlace(
      long parkingId, long placeNumber, String userEmail) { // TODO check for book one place
    ParkingPlace pp = getParkingPlace(parkingId, placeNumber);
    User user = userService.findByEmail(userEmail);
  ...
  public Parking addParking(ParkingDto parkingDto) {
    ...
    Parking parking =
        new Parking(parkingDto.getX(), parkingDto.getY(), parkingDto.getPricePerHour());
